    ...
    <div class="px-4 sm:px-8 py-6">
      <div class="flex items-center justify-between mb-6">
        <h1 class="section-title">
          <span class="inline-block w-4 h-4 rounded-full bg-red-pet mr-2 align-middle" aria-hidden="true"></span>
          Animales Perdidos
        </h1>
        <a routerLink="/reportar" class="btn-red text-sm">+ Publicar</a>
      </div>

      <app-report-filter [initialStatus]="'LOST'" (filterChange)="onFilter($event)" />
      ...
      @if (loading()) {
        ...
        <app-empty-state
          icon="🔍"
          title="No hay mascotas perdidas"
          message="No se encontraron publicaciones de mascotas perdidas con esos filtros."
          actionLabel="Publicar una"
          actionLink="/reportar" />
      } @else {
...
export class LostComponent implements OnInit {
  ...
  filter   = signal<ReportFilter>({ status: 'LOST', page: 0, size: 9 });
  ...
  onFilter(partial: Partial<ReportFilter>): void {
    this.filter.set({ ...this.filter(), ...partial, status: 'LOST', page: 0 });
    this.load();
  }
